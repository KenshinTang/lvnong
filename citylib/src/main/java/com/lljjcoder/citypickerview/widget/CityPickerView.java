...
//        initProvinceDatas(context);
    ...
    protected void parserJsonData(){
        ...
        try{
            ...
            JSONArray ja = new JSONArray(a);//最外面的Json数组
            JSONObject shenjb = ja.getJSONObject(0);//取出第一个省份的Json数据
            //首先初始化默认选中的省市区
            mCurrentProviceName = shenjb.getString("name");//取出省份名
            JSONArray sheJa = shenjb.getJSONArray("city");//根据省份名取出市的json数组
            mCurrentCityName = sheJa.getJSONObject(0).getString("name");//取出市json数组的市名称
            JSONArray quJa = sheJa.getJSONObject(0).getJSONArray("area");//取出区json数组
            mCurrentDistrictName = quJa.getString(0);//取出区名称

            mProvinceDatas = new String[ja.length()];
            for (int i = 0; i < ja.length(); i++) {
                // 遍历所有省的数据i
                mProvinceDatas[i] = ja.getJSONObject(i).getString("name");//取出所有的省
                JSONArray resultCityJa = ja.getJSONObject(i).getJSONArray("city");//根据省取出市的json数组
                String[] cityNames = new String[resultCityJa.length()];
                for (int j = 0; j < resultCityJa.length(); j++) {
                    // 遍历省下面的所有市的数据
                    JSONObject resultCityJb = resultCityJa.getJSONObject(j);
                    cityNames[j] = resultCityJb.getString("name");//取出所有的市
                    JSONArray resultQuJa = resultCityJb.getJSONArray("area");//根据省取出市的json数组
                    String[] distrinctNameArray = new String[resultQuJa.length()];
                    for (int k = 0; k < resultQuJa.length(); k++) {
                        // 遍历市下面所有区/县的数据
                        distrinctNameArray[k] = resultQuJa.getString(k);//取出所有的区
                    }
                    // 市-区/县的数据，保存到mDistrictDatasMap
                    mDistrictDatasMap.put(cityNames[j], distrinctNameArray);
                }
                // 省-市的数据，保存到mCitisDatasMap
                mCitisDatasMap.put(ja.getJSONObject(i).getString("name"), cityNames);
            }
        }catch (Exception e){
